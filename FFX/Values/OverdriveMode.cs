...
namespace Farplane.FFX.Values
{
    public class OverdriveMode
    {
        ...
        {
            var odOffset = _offsetParty + (int) PartyStatOffset.OverdriveModes + charIndex*0x94;
            var odBytes = MemoryReader.ReadBytes(odOffset, 3);
            ...
            MemoryReader.WriteBytes(odOffset, odBytes);
        }

        public static void SetOverdriveCounter(int charIndex, int odIndex, int odCount)
        {
            var odOffset = _offsetParty + (int) PartyStatOffset.OverdriveWarrior + charIndex*0x94 + OverdriveModes[odIndex].BitIndex * 2;
            MemoryReader.WriteBytes(odOffset, BitConverter.GetBytes((ushort)odCount));
        }
